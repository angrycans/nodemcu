...
// void init_queue()
// {
//     queue_handle = xQueueCreate(5, sizeof(String));
...
void taskdDownloadfile(void *parameter)
{
    ...
    // uint32_t len;
    // uint32_t crc = crc32(f, len);
    // f.close();
    // Serial.print("CRC32 is 0x");
    // Serial.println(crc, HEX);
    // Serial.print("File length was ");
    // Serial.println(len);
    ...
    {
        // String fileinfo = "";
        // fileinfo += myFile.size();

        // pCharacteristicDownloadFile->setValue(fileinfo.c_str());
        ...
        rst += ":crc32:" + String(~oldcrc32) + ":" + String(millis() - startMillis);
    }
    else
    {
        rst += ":error";
...
void HAL::BLE_Init()
{
    ...
    pCharacteristicDownloadFile = pService->createCharacteristic(SERVICE_UUID("0004"), BLECharacteristic::PROPERTY_WRITE);
