/*
感谢原始版本开源代码 https://github.com/cubexvr/servoController
以及在此基础上修改的带行程检测版本 https://github.com/ehmotion/eh-msc-4dof
以下代码是本人结合以上代码，修改为自动回0位，然后直接抬致行程中间的版本。使接线更容易，初始化电缸更安全合理。
*/
#include <Arduino.h>
...
void setup()
{
  ...
  int m_count = motors_count();

  debug_print("motors_count=");
  debug_println(m_count);
...
int homing_checkX(int X)
{
  ...
  else if (motor_state[X] == ST_CTRING)
  {
    ...
    if (currentPos[X] <= motor_mins[X])
    {
      ...
      uint32_t ctr_pos = ((uint32_t)motor_mins[X] + (uint32_t)motor_maxs[X]) / 2;
      targetInput[X] = (uint16_t)motor_mins[X] + mmove_try[X];
    }
    else
    {
